...
`PrometheuxClient` speaks JSON-RPC to the backend. Rather than exposing one
method per tool, it has a single `rpc()` call — you name the tool and pass its
arguments, exactly as an MCP client would.
...
async def main():
    ...
    try:
        # Discover what the backend currently exposes
        catalog = await client.rpc("tools/list", {})
        print(f"{len(catalog['tools'])} tools available")

        # List concepts
        concepts = await client.rpc("tools/call", {
            "name": "list_concepts",
            "arguments": {"ontology_id": "ontology-123"},
        })
        print(concepts)

        # Run a concept
        result = await client.rpc("tools/call", {
            "name": "run_concept",
            "arguments": {
