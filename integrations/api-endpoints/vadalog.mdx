...
<Tip>
For concept-level execution (with dependency chaining, result persistence, and progress tracking), use [`POST /concepts/{ontology_id}/run/{concept_name}`](/integrations/api-endpoints/concepts#run-a-concept) instead. Use `/vadalog/evaluate` when you want to execute a program directly without a saved concept.
</Tip>
