...
Graph-building endpoints are [paginated](/integrations/rest-api#pagination).
...
```bash
curl -X POST "$BASE_URL/kgs/$PROJECT_ID/build-graph/edge?page=1&page_size=50" \
...
```bash
curl -X POST "$BASE_URL/kgs/$PROJECT_ID/graph-analytics/edge" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
