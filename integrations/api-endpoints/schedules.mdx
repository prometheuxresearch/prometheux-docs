...
```bash
curl "$BASE_URL/schedules/$PROJECT_ID/policies" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl "$BASE_URL/schedules/$PROJECT_ID/policies/$POLICY_ID" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X DELETE "$BASE_URL/schedules/$PROJECT_ID/policies/$POLICY_ID" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X POST "$BASE_URL/schedules/$PROJECT_ID/policies/$POLICY_ID/trigger" \
  -H "Authorization: Bearer $TOKEN"
```
