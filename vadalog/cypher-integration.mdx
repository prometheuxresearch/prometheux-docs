...
<Note>
**Cypher cells in the Prometheux platform**

The Prometheux [platform](/platform/ontologies) authors programs as **cells**, each with a type — `vadalog`, `cypher`, `sql`, or `python` (set via the [concept editor toolbar](/platform/concept-editor#toolbar)). In a **Cypher cell** you write the query directly, without wrapping it in a Vadalog rule head:
...
Algorithm parameters (`relationshipWeightProperty`, `sourceNode`, `dampingFactor`) are passed through to the function. Single-source shortest path and BFS take a `sourceNode` that scopes the computation to that start node, and both report one row per reachable couple as `(source, target, distance)` — the source is constant for a single-source run, and for BFS the distance is the integer hop count (level):

```prolog
sssp(Source, Target, Distance) <-
    CALL gds.graph.project('g', 'Person', 'KNOWS')
...
bfs(Source, Target, Level) <-
    CALL gds.graph.project('g', 'Person', 'KNOWS')
    CALL gds.bfs.stream('g', {sourceNode: 'alice'})
    YIELD nodeId
    RETURN nodeId.
...
- **AI / vector-search functions**: `embeddings:vectorize`, `similarity:cosine_sim`, `llm:generate`, and the `ask(...)` function for vector retrieval over Qdrant collections.
