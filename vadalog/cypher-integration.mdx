...
The Prometheux [platform](/platform/projects) authors programs as **cells**, each with a type — `vadalog`, `cypher`, `sql`, or `python` (set via the [concept editor toolbar](/platform/concept-editor#toolbar)). In a **Cypher cell** you write the query directly, without wrapping it in a Vadalog rule head:
