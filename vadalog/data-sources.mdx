---
title: "Data Sources"
description: "Connecting Vadalog to CSV, Parquet, Iceberg, Excel, JSON, COBOL, PostgreSQL, Neo4j, DynamoDB, Qdrant, RDF triple stores, S3, HDFS, REST APIs, and many more data sources."
---
...
| `rdf` | RDF serializations (N-Triples / N-Quads / Turtle / RDF/XML / JSON-LD) for RDF triple stores |
...
Iceberg-bound predicates accept **inline Cypher queries** in rule bodies the same way they accept SQL bodies. You write a Cypher pattern; the engine produces an execution plan that pushes filters, projections, partition pruning and file skipping into the Iceberg scan, then runs the join / aggregation / graph algorithm over the pruned data. There is no Neo4j server in the loop — the engine evaluates the Cypher locally against the Iceberg tables, and snapshots, branches, schema evolution and statistics work exactly as they do for plain Iceberg reads.

The full Cypher language surface — patterns, `WHERE` operators, `RETURN` expressions, `WITH` pipelines, aggregations, GDS algorithms — is documented once for every source in [Cypher Integration](/vadalog/cypher-integration). This section focuses on the Iceberg-specific aspects: how a Cypher pattern lines up with an Iceberg table, what pushdown looks like, and how time travel / branches / mixed-source joins compose with Cypher bodies.

#### Property-graph ↔ Iceberg layout

For a Cypher pattern to line up with an Iceberg table, follow the property-graph ↔ relational convention used by every non-Neo4j source (see [Cypher Integration › How Cypher patterns map to your data](/vadalog/cypher-integration#how-cypher-patterns-map-to-your-data) for the full reference):

| Cypher element | Iceberg table layout |
| --- | --- |
| Node label `(:Person)` | a table `person` (lower-cased) with an `id` column |
| Relationship type `[:KNOWS]` | a table `knows` (lower-cased) with `src` / `dst` columns referencing node ids |
| Property `p.age` | a column `age` |
| Property of a relationship `r.weight` | a column `weight` on the relationship table |
...
```prolog
% A person table (id, name, age, city, balance, tags, since, ...).
@bind("person", "iceberg", "prod_catalog.graph", "person").

% A knows relationship (src, dst, since, weight) — endpoints reference person ids.
...
```prolog
adult_londoners(Name) <-
    MATCH (p:Person)
    WHERE p.age >= 18 AND p.city = "London"
    RETURN p.name AS name.

@output("adult_londoners").
```
...
```prolog
% Only edges among `person` ids are scored; nodes that aren't in the person
% table are dropped before PageRank runs.
pagerank_persons(Node, Score) <-
    CALL gds.graph.project('g', 'Person', 'KNOWS')
    CALL gds.pageRank.stream('g')
    YIELD nodeId, score
    RETURN nodeId, score.

@output("pagerank_persons").
```

See [Cypher Integration › Graph algorithms](/vadalog/cypher-integration#graph-algorithms-gds) for the full algorithm catalog and parameter options.
...
```prolog
% Same Cypher, run against the orders table as it looked at midnight UTC on 2026-05-01.
@bind("orders_may1", "iceberg as-of-timestamp='1746057600000'",
      "prod_catalog.sales", "orders").
...
high_value_then(O, A) <-
    ...
    RETURN o.order_id AS o, o.amount AS a.

@output("high_value_then").
```
...
```prolog
@bind("knows_dev", "iceberg branch='dev'", "prod_catalog.graph", "knows").

dev_pagerank(Node, Score) <-
    CALL gds.graph.project('g', 'Person', 'KNOWS_DEV')
    CALL gds.pageRank.stream('g')
    YIELD nodeId, score
    RETURN nodeId, score.
...
For the rest of the Cypher surface — list / map / temporal operations, `WITH` pipelines, the full `WHERE` and `RETURN` expression library, and recipes for composing Vadalog rules on top of a Cypher result — see [Cypher Integration](/vadalog/cypher-integration).

### Vadalog, SQL, and Cypher together over Iceberg

The same Iceberg dataset can be queried with **plain Vadalog rules**, **inline SQL bodies**, or **inline Cypher bodies**. Pick the lens that best fits the query you're writing — they all share the same Iceberg planning-time pushdown (partition pruning, file skipping, column pruning) and they all compose with the rest of your program.

Suppose your catalog contains an Iceberg `person` table (id, name, age, city) and an Iceberg `knows` table (src, dst, since). Three identical questions, expressed three ways:
...
adult(Id, Name) :- person(Id, Name, Age, _), Age > 18.

adult_friend(AName, BName) :-
    adult(A, AName),
    knows(A, B, _),
    person(B, BName, _, _).
...
- **Vadalog** — recursion, derivations, the chase, AI / vector functions, hashing, anything the other lenses cannot express. See [Thinking in Vadalog](/vadalog/thinking-in-vadalog).
- **SQL** — joins, aggregations, window functions, CTEs, anything that is naturally relational. See [SQL Integration](/vadalog/sql-integration).
- **Cypher** — pattern matching, multi-hop traversals, `shortestPath`, GDS algorithms. See [Cypher Integration](/vadalog/cypher-integration).
</Tip>
...
The same inline Cypher also runs over **non-Neo4j** sources — CSV/Parquet/Iceberg files, relational databases, vector stores, and derived predicates. See [Cypher Integration](/vadalog/cypher-integration) for the full language surface, or [Cypher over non-Neo4j sources](#cypher-over-non-neo4j-sources-csv-files-databases) below for the layout convention.
</Note>
...
A Cypher body is **not limited to Neo4j**. The same inline Cypher also runs when the predicates it references are bound to CSV / Parquet / Iceberg files, a relational database (PostgreSQL, MariaDB, Snowflake, …), in-memory facts, or even a **derived predicate produced by another rule**. The full Cypher language surface — patterns, `WHERE` operators, `RETURN` expressions, `WITH` pipelines, aggregations, GDS algorithms — is documented in [Cypher Integration](/vadalog/cypher-integration). This section focuses on what is specific to non-Neo4j sources: how a Cypher pattern lines up with relational tables and files.
...
| Node or relationship property `p.age` | a column `age` by the same name |

The field names used in Cypher must match a CSV header or a table column — if there is no corresponding name, the query errors with `column not found`. Use [`@mapping`](/vadalog/annotations#mapping) to alias source columns into this layout when the underlying schema does not match.
...
For the full Cypher surface — `WHERE` operators, projection / value transformation, aggregation, `WITH` pipelines, graph algorithms, and worked examples over each source kind — see [Cypher Integration](/vadalog/cypher-integration).
...
Prometheux can serialize a reasoning result to **RDF** files on disk or object storage, ready for bulk import into any RDF triple store. The connector is powered by [Apache Jena](https://jena.apache.org/), so IRI and literal escaping follow the RDF 1.1 grammar rather than a hand-rolled encoder.

The `rdf` connector is **write-only** (an export target): you bind it on an `@output` concept exactly like the Parquet writer, using the same `<schema>/<table>` path convention. By default the output is a **directory** of RDF files plus a `_SUCCESS` marker — one file per Spark partition — so large results are written in parallel across the cluster. Set [`singleFile=true`](#bind-options-for-rdf) to instead emit **one correctly named file** (`<table>.ttl` / `.rdf` / `.jsonld` / `.nt` / `.nq`) that is trivial to hand off.

The produced files are **standard, W3C-conformant RDF** (there is no Prometheux-specific framing), so any RDF platform can ingest them directly with its native bulk loader — see [Loading into a triple store](#loading-into-a-triple-store).

The same row-to-triple mapping can also target a **relational or warehouse connector** (PostgreSQL, Databricks, Snowflake, …) instead of files, by adding `outputFormat=rdf` to that connector's `@bind` — see [Writing RDF triples to a database](#writing-rdf-triples-to-a-database).

### Serialization formats

Choose the serialization with the `format` option:

| `format` | Description | Parallelism |
|----------|-------------|-------------|
| `ntriples` (default) | Line-oriented N-Triples — one triple per line. Aliases: `nt`, `n-triples`. | Fully distributed (one triple line per row, written independently). |
| `nquads` | Line-oriented N-Quads — adds a named-graph term (see `graphIri`). Aliases: `nq`, `n-quads`. | Fully distributed. |
| `turtle` | Human-readable Turtle — one self-contained document per partition. Aliases: `ttl`. | Distributed per partition; each partition builds an in-memory graph. |
| `rdfxml` | RDF/XML — one self-contained `<rdf:RDF>` document per partition. Aliases: `rdf/xml`, `xml`, `owl`. | Distributed per partition; each partition builds an in-memory graph. |
| `jsonld` | JSON-LD 1.1 — one self-contained JSON document per partition. Aliases: `json-ld`, `jsonld11`. | Distributed per partition; each partition builds an in-memory graph. |
...
How each row becomes triples is driven by the [`@model`](/vadalog/annotations#model) annotation and the `mode` option:
...
- `emitOntology`: whether to also emit schema-level (TBox) triples derived from the `@model` — `rdfs:Class`/`rdfs:subClassOf`/`rdfs:comment` for entities, or `rdf:Property` with `rdfs:domain`/`rdfs:range` for triples (default: `true`).
...
In this mode the subject column reuses the shared `subjectColumn` option, and `emitOntology` controls whether the inline `rdf:type` triples are emitted.
...
@bind("person_csv", "csv useHeaders=true", "disk/data/input", "people.csv").

% Describe the output concept: 'id' is the subject key
@model("person", "['id(ID):string', 'name:string', 'age:int']").

person(Id, Name, Age) <- person_csv(Id, Name, Age).

@output("person").

% Write N-Triples under <baseIri> to the directory 'rdf_out/person'
@bind("person",
      "rdf format='ntriples', baseIri='http://acme.example/', mode='entity'",
      "rdf_out", "person").
```
...
```turtle
<http://acme.example/person> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://www.w3.org/2000/01/rdf-schema#Class> .
<http://acme.example/person/p1> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://acme.example/person> .
<http://acme.example/person/p1> <http://acme.example/name> "Alice" .
<http://acme.example/person/p1> <http://acme.example/age> "30"^^<http://www.w3.org/2001/XMLSchema#integer> .
```
...
```prolog
@bind("knows_csv", "csv useHeaders=true", "disk/data/input", "knows.csv").

% The endpoint class ...
@model("person", "['id(ID):string']").
% ... and the relationship: person --knows--> person
@model("(person)knows(person)", "['src(sID):string', 'dst(tID):string', 'since:string']").

knows(Src, Dst, Since) <- knows_csv(Src, Dst, Since).

@output("knows").

@bind("knows",
      "rdf format='ntriples', baseIri='http://acme.example/'",
      "rdf_out", "knows").
```
...
```turtle
<http://acme.example/knows> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://www.w3.org/1999/02/22-rdf-syntax-ns#Property> .
<http://acme.example/knows> <http://www.w3.org/2000/01/rdf-schema#domain> <http://acme.example/person> .
<http://acme.example/knows> <http://www.w3.org/2000/01/rdf-schema#range> <http://acme.example/person> .
<http://acme.example/person/alice> <http://acme.example/knows> <http://acme.example/person/bob> .
```
...
% Turtle
@bind("person",
      "rdf format='turtle', baseIri='http://acme.example/'",
      "rdf_out", "person_ttl").

% RDF/XML
@bind("person",
      "rdf format='rdfxml', baseIri='http://acme.example/'",
      "rdf_out", "person_rdf").

% JSON-LD
@bind("person",
      "rdf format='jsonld', baseIri='http://acme.example/'",
      "rdf_out", "person_jsonld").

% N-Quads into a named graph
@bind("person",
      "rdf format='nquads', baseIri='http://acme.example/', graphIri='http://acme.example/graph/people'",
      "rdf_out", "person_nq").
```
...
```prolog
% One 'person.ttl' file (instance triples + the @model ontology header) under 'rdf_out'
@bind("person",
      "rdf format='turtle', singleFile=true, baseIri='http://acme.example/'",
      "rdf_out", "person").

% One 'person.jsonld' file
@bind("person",
      "rdf format='jsonld', singleFile=true, baseIri='http://acme.example/'",
      "rdf_out", "person").
```
...
| alice   | Person       | name      | Alice  | string      |
| alice   | Person       | age       | 30     | int         |
| alice   | Person       | livesIn   | rome   | City        |
| rome    | City         | name      | Rome   | string      |

```prolog
@bind("facts_csv", "csv useHeaders=true", "disk/data/input", "facts.csv").

facts(S, ST, P, O, OT) <- facts_csv(S, ST, P, O, OT).

@output("facts").
% The @model only names the five columns so the *Column options can reference them
@model("facts", "['subject:string', 'subject_type:string', 'predicate:string', 'object:string', 'object_type:string']").

@bind("facts",
      "rdf format='ntriples', baseIri='http://acme.example/',
           subjectColumn='subject', subjectTypeColumn='subject_type',
           predicateColumn='predicate',
           objectColumn='object', objectTypeColumn='object_type'",
      "rdf_out", "facts").
```
...
```turtle
<http://acme.example/Person/alice> a                       <http://acme.example/Person> .
<http://acme.example/Person/alice> <http://acme.example/knows>   <http://acme.example/Person/bob> .
<http://acme.example/Person/bob>   a                       <http://acme.example/Person> .
<http://acme.example/Person/alice> <http://acme.example/name>    "Alice" .
<http://acme.example/Person/alice> <http://acme.example/age>     "30"^^<http://www.w3.org/2001/XMLSchema#integer> .
<http://acme.example/Person/alice> <http://acme.example/livesIn> <http://acme.example/City/rome> .
<http://acme.example/City/rome>    a                       <http://acme.example/City> .
<http://acme.example/City/rome>    <http://acme.example/name>    "Rome" .
```
...
This already produces a valid graph — two `Person` entities linked by `knows`:

```turtle
<http://acme.example/Person/alice> <http://acme.example/knows> <http://acme.example/Person/bob> .
<http://acme.example/Person/bob>   <http://acme.example/knows> <http://acme.example/Person/carol> .
```
...
Notice `alice` is the object of a `knows` triple *and* the subject of her own `name`/`age` triples — that is what ties the graph together.
...
| subject | subject_type | predicate | object | object_type |
|---------|-------------|-----------|--------|-------------|
| f1      | Friendship  | from      | alice  | Person      |
| f1      | Friendship  | to        | bob    | Person      |
| f1      | Friendship  | since     | 2019   | int         |

```turtle
<http://acme.example/Friendship/f1> a                        <http://acme.example/Friendship> .
<http://acme.example/Friendship/f1> <http://acme.example/from>  <http://acme.example/Person/alice> .
<http://acme.example/Friendship/f1> <http://acme.example/to>    <http://acme.example/Person/bob> .
<http://acme.example/Friendship/f1> <http://acme.example/since> "2019"^^<http://www.w3.org/2001/XMLSchema#integer> .
```
...
Example — write the generic triple table from [Example 4](#example-4-generic-triple-table-column-driven) into a PostgreSQL table instead of files. Only the connector and the `outputFormat=rdf` flag change; the RDF options are identical:
...
% Target is a PostgreSQL table 'facts_triples'; the RDF options are the same as the file writer
@bind("facts",
      "postgresql outputFormat=rdf, format=ntriples, baseIri='http://acme.example/',
           subjectColumn='subject', subjectTypeColumn='subject_type',
           predicateColumn='predicate',
           objectColumn='object', objectTypeColumn='object_type'",
      "my_database", "facts_triples").
```

The resulting `facts_triples` table holds one row per triple (IRIs are stored as their full string, literals as their bare lexical value):

| subject | predicate | object |
|---------|-----------|--------|
| http://acme.example/Person/alice | http://www.w3.org/1999/02/22-rdf-syntax-ns#type | http://acme.example/Person |
| http://acme.example/Person/alice | http://acme.example/knows | http://acme.example/Person/bob |
| http://acme.example/Person/alice | http://acme.example/name | Alice |
| http://acme.example/Person/alice | http://acme.example/age | 30 |
| … | … | … |

Because it is an ordinary table, you can read it back with the plain connector and continue reasoning over the triples:

```prolog
@bind("facts_triples", "postgresql", "my_database", "facts_triples").

triple(S, P, O) <- facts_triples(S, P, O).
...
The output contains standard, W3C-conformant RDF files, so any triple store can ingest them with its native bulk-load facility (e.g. Fuseki `tdb2.tdbloader`, GraphDB import, the rdf4j console, or a cloud bulk loader) or over the [SPARQL 1.1 Graph Store HTTP Protocol](https://www.w3.org/TR/sparql11-http-rdf-update/). Point the loader at the whole directory, or upload files individually; in the default distributed mode each part file is a **complete, standalone document**, and with `singleFile=true` you get a single ready-to-load file.

Set the request `Content-Type` to match the `format` you wrote:

| `format` | Media type | Single-file extension |
|----------|-----------|-----------------------|
...
```bash
# Written with: rdf format='turtle', singleFile=true  ->  rdf_out/person.ttl
curl -X POST \
     -H "Content-Type: text/turtle" \
     --data-binary @rdf_out/person.ttl \
     "http://localhost:5820/myDb?graph=http://acme.example/graph/people"
```
...
curl -X POST \
     ...
     --data-binary @rdf_out/person/part-00000.txt \
     "http://localhost:5820/myDb?graph=http://acme.example/graph/people"
```
...
```prolog
% Read from a Unity Catalog table with no credentials
@bind("sales", "databricks inCluster=true", "main.sales", "transactions").
report(Product, Total) <- SELECT product_id, sum(amount) AS total FROM sales GROUP BY product_id.
@output("report").

% Write back into Unity Catalog (saveAsTable); saveMode='overwrite' replaces the table
@bind("report", "databricks inCluster=true, saveMode=overwrite", "main.sales", "product_totals").
```
